...
deletionProbability = [0 0.25 0.3 0.35 0.4 0.55];
fillGaps = 'iterative'; % 'none' 'incidences' 'iterative'
saveFigs = false;

resultsSupineHRF = cell(length(subject),length(deletionProbability));
resultsTiltHRF = cell(length(subject),length(deletionProbability));
...
if saveFigs, saveas(gcf,strcat(['images\deg_rd_hrf_' fillGaps '_pip']),'epsc'); end
...
RMSE = sqrt(nansum((aux2-aux1).^2,1)./(2*length(subject))) % RMSE
% nRMSE = sqrt(nansum((aux2-aux1).^2,1)./(2*length(subject)))./nanmean(aux2(:,1)) %#ok<*NOPTS> % nRMSE
if saveFigs, saveas(gcf,strcat(['images\deg_rd_hrf_' fillGaps '_ials']),'epsc'); end
...
RMSE = sqrt(nansum((aux2-aux1).^2,1)./(2*length(subject))) % RMSE
% nRMSE = sqrt(nansum((aux2-aux1).^2,1)./(2*length(subject)))./nanmean(aux2(:,1)) %#ok<*NOPTS> % nRMSE
if saveFigs, saveas(gcf,strcat(['images\deg_rd_hrf_' fillGaps '_pss']),'epsc'); end
...
RMSE = sqrt(nansum((aux2-aux1).^2,1)./(2*length(subject))) % RMSE
% nRMSE = sqrt(nansum((aux2-aux1).^2,1)./(2*length(subject)))./nanmean(aux2(:,1)) %#ok<*NOPTS> % nRMSE
if saveFigs, saveas(gcf,strcat(['images\deg_rd_hrf_' fillGaps '_pas']),'epsc'); end
...
ylabel('PIP [%]')
if saveFigs, saveas(gcf,strcat(['images\ans_rd_hrf_' fillGaps '_pip']),'epsc'); end
...
fancyBoxplot(aux1,aux2,100*deletionProbability,significance,true)
ylabel('IALS [intervals^{-1}]')
if saveFigs, saveas(gcf,strcat(['images\ans_rd_hrf_' fillGaps '_ials']),'epsc'); end
...
fancyBoxplot(aux1,aux2,100*deletionProbability,significance,true)
ylabel('PSS [%]')
if saveFigs, saveas(gcf,strcat(['images\ans_rd_hrf_' fillGaps '_pss']),'epsc'); end
...
fancyBoxplot(aux1,aux2,100*deletionProbability,significance,true)
ylabel('PAS [%]')
if saveFigs, saveas(gcf,strcat(['images\ans_rd_hrf_' fillGaps '_pas']),'epsc'); end
